...
async function findProductById(id) {
    console.log(`Recherche produit BDD ID: ${id}`);
    try {
        const product = await Product.findById(id); // Cherche en BDD
        console.log(`Produit trouvé BDD: ${product ? product.name : 'Non trouvé'}`);
        return product;
    } catch (error) {
        if (error.name === 'CastError') { console.warn(`ID produit BDD invalide: ${id}`); return null; }
        console.error("findProductById:", error); throw error;
    }
}
...
exports.showHome = async (req, res) => {
    console.log("Accès contrôleur showHome");
    ...
    try {
        ...
        console.log(`${products.length} produits trouvés en BDD pour l'accueil.`);
        if (products.length === 0) {
             console.warn("Aucun produit trouvé dans la collection 'products'. Vérifie ta BDD !");
        }
        res.render('index', {
            products: products,
            message: message
...
exports.addToCart = async (req, res) => {
    ...
    console.log(`addToCart - ID Reçu: ${productId}`);
    try {
        ...
        if (existingItemIndex > -1) { // Déjà présent -> Incrémente quantité
            ...
            console.log(`Quantité incrémentée pour ${product.name}.`);
        ...
        console.log('Panier SESSION OBJET après ajout/modif');
        ...
        req.session.save(err => {
            if (err) { console.error("save session addToCart:", err); }
             // Réponse AJAX ou redirection
            if (req.headers.accept && req.headers.accept.includes('application/json')) {
                console.log("Réponse JSON envoyée pour AJAX.");
